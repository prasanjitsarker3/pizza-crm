...
export const ProductEdit = ({ productData }: ProductEditProps) => {
    const router = useRouter();
    const { data, isLoading: loadingCategories } = useGetCategoryQuery({});
    ...
    useEffect(() => {
        if (productData) {
            ...
            setSpicyLevels(productData.spicyLevel || []);
            setExistingImages(productData.mainImages || []);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response?.statusCode === 200) {
                ...
                toast.error(response?.message || response?.data?.message || "Failed to update product", {
                    id: toastId,
                });
            }
        } catch (error: any) {
                ...
                <div className="mt-6">
                    <form onSubmit={handleSubmit} className="space-y-8">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-2">
                                <Input
                                    ...
                                    className="h-11"
                                />
                                <Button type="button" onClick={addIngredient} className="px-6">
                                    Add
                                </Button>
                            </div>
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        {existingImages.map((img, index) => (
                                            <div
                                                key={`existing-${index}`}
                                                className="relative group rounded-lg overflow-hidden border-2 border-gray-200 hover:border-primary"
                                            >
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        {previewImages.map((img, index) => (
                                            <div
                                                key={`new-${index}`}
                                                className="relative group rounded-lg overflow-hidden border-2 border-blue-200 hover:border-blue-400"
                                            >
                                                <div className="relative w-full h-40">
                                                    <Image
                                                        src={img}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
                                {categoryData.map((category) => (
                                    <div
                                        key={category.id}
                                        className="flex items-center space-x-2 bg-gray-50 px-4 py-3 rounded-lg hover:bg-gray-100"
                                    >
                                        <Checkbox
                                            id={category.id}
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t">
                            ...
                            >
                                {isUpdating || uploadingImages ? (
                                    <>
                                        <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
                                        {uploadingImages ? "Uploading Images..." : "Updating Product..."}
                                    </>
                                ) : (
                                    "Update Product"
                                )}
