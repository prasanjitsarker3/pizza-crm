...
interface FilteringUserProps {
  filter: IFilterUser;
  setFilter: React.Dispatch<React.SetStateAction<IFilterUser>>;
}

const FilteringUser = ({ filter, setFilter }: FilteringUserProps) => {
  ...
  const rolesData: IRole[] = data?.data || [];
  ...
  const hasActiveFilters =
    (filter.roleId && filter.roleId !== "all") ||
    (filter.isVerified && filter.isVerified !== "all") ||
    filter.startDate ||
    filter.endDate;
      ...
      <PopoverContent className="w-80 dark:bg-[#1F1F1F]" align="end">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h4 className="font-semibold text-sm">Filter Users</h4>
            {hasActiveFilters && (
              <Button
                variant="ghost"
                size="sm"
          ...
          <div className=" grid grid-cols-2 gap-2">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={filter.isVerified || "all"}
                onValueChange={(value) =>
                  setFilter({ ...filter, isVerified: value })
                }
              >
                <SelectTrigger
                  id="verification"
          ...
          {/* Apply Button */}
          <Button onClick={handleApplyFilters} className="w-full dark:bg-[#ff7200] dark:text-white">
            Apply Filters
          </Button>
        </div>
      </PopoverContent>
